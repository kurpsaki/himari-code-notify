...
| `UserApproval` | (なし・論理イベント) | ユーザー許可が必要 — `notify.mjs UserApproval` で直接呼び出すか Codex CLI から使用。Claude Code の `settings.json` フックキーとしては現在未対応。 |

> **zundamon-code-notify から移行する場合**: `~/.claude/settings.json` やプロジェクトの `.claude/settings.json` に zundamon-code-notify の hook エントリが残っていると、**2 つの hook が同時に発火して二重通知**になります。himari-code-notify に切り替える際は、先に zundamon-code-notify のエントリをすべて削除してください。`scripts/setup.sh` を実行すると競合の有無を自動チェックします。
